...
function scheduleFor(modeId){
  ...
  const week = {};
  // Every weekday alike, which is what the flat config meant. Sunday included:
  // the old config had no way to say otherwise, and inventing a closed Sunday
  // here would change behaviour for a clinic that does open on Sundays.
  for(let d = 0; d < 7; d++) week[d] = day.map(function(iv){ return iv.slice(); });
  return {
