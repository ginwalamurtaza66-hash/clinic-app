...
/* Mirrors applyTypography() in index.html. The two cannot share code -- this
   page has no module in common with the app -- so the stacks and scales are
   repeated here and a test asserts the two lists agree. */
/* NOT the same stacks as the app, on purpose. This page loads no webfonts at
   all -- that was decided in 3.24.0, because the app needs to work offline and
   this page is a one-off view on a patient's mobile data, where 445 KB of fonts
   would be paid for by every visitor. So Fraunces is not available here and
   falls back to Georgia, and the body has always been the device's own font.

   The choice offered is therefore between a serif heading and one plain face
   throughout, which is a real difference a clinic can see, rather than three
   options where two are identical. */
const SYS = "system-ui,-apple-system,'Segoe UI',Roboto,sans-serif";
const SERIF = "Georgia,'Times New Roman',serif";
/* `default` is exactly what this page already looked like -- the device's own
   font throughout -- so a clinic that never touches the setting sees no change
   at all. The other two are real, visible differences rather than three options
   where two render identically. */
const FONT_STACKS = {
  default:  { display:SYS,   body:SYS },
  headings: { display:SERIF, body:SYS },
  serif:    { display:SERIF, body:SERIF }
};
/* Capped at 1.2, measured rather than chosen. Every size in these files is
   in px, so zoom is the only lever that moves them; at 1.25 the layout is
   still clean and at 1.3 it overflows by about 5px on a 390px phone -- five
   pixels being invisible as a design flaw and very obvious as a page that
   slides sideways under your thumb. */
/* Chosen so 'largest' is worth choosing. 1.3 leaves about 5px of zoom
   rounding, which is why overflow-x is clipped when a scale is applied --
   invisible as trimmed padding, very visible as a sideways scroll.
   These four values MUST match the copy in the other file. */
/* Capped at 1.2, measured rather than chosen. Every size in these files is
   in px, so zoom is the only lever that moves them; at 1.25 the layout is
   still clean and at 1.3 it overflows by about 5px on a 390px phone -- five
   pixels being invisible as a design flaw and very obvious as a page that
   slides sideways under your thumb. */
const UI_SCALES = { small:0.9, default:1, large:1.1, largest:1.2 };

function applyTypography(cfg) {
  const c = cfg || {};
  const stack = FONT_STACKS[c.font] || FONT_STACKS.default;
  const scale = UI_SCALES[c.fontSize] || 1;
  const root = document.documentElement;
  root.style.setProperty('--font-display', stack.display);
  root.style.setProperty('--font-body', stack.body);
  root.style.setProperty('--wt-body', c.fontBold ? '500' : '400');
  document.body.style.zoom = scale === 1 ? '' : String(scale);
  const hide = scale === 1 ? '' : 'hidden';
  document.body.style.overflowX = hide;
  // The guard belongs on BOTH: the element that actually scrolls is
  // documentElement, so hiding overflow on body alone left the page sliding
  // sideways while looking as though it had been handled.
  document.documentElement.style.overflowX = hide;
}
